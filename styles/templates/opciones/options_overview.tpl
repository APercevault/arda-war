...
<div id="eins_small">
 ...
 <br /> 
    ...
    <tr>
        ...
        <td>{$op_username}</td>
        <td style="height:22px;">{if $uctime}<input class="campo_comun" name="db_character" size=""  maxlength="15" value="{$opt_usern_data}" type="text">{else}{$opt_usern_data}{/if}</td>
    </tr><tr>
        <td>{$op_old_pass}</td>
        ...
        <td>{$op_lang}</td>
        <td>
			{html_options name=langs style="background-color: black;" options=$Selectors.lang selected=$langs}
        </td>
    </tr><tr>
        <td>{$op_sort_planets_by}</td>
        <td>
			{html_options name=settings_sort style="background-color: black;" options=$Selectors.Sort selected=$planet_sort}
        </td>
    </tr><tr>
		<td>{$op_sort_kind}</td>
        <td>
            {html_options name=settings_order style="background-color: black;" options=$Selectors.SortUpDown selected=$planet_sort_order}
        </td>
    </tr><tr>
	...
	<tr>
        <td colspan="2"><center><input class="submitb" value="{$op_save_changes}" type="submit"></td>
    </tr>
    </tbody>
    </table>
    </form>
...
<div id="menu_flotas">
		<div id="lista_misiones">    
			...
			<div class="misiones">
			<img class="tooltip" name="<img src={$user_avatar}  width=300 height=300 />" src="{$user_avatar}" width="147" height="147" /></td>
			</div> 
		   ...
		   <form action="?page=options&amp;mode=avatar" method="post">
		<input class="submit" type="submit" onclick=return false;" value="{$op_modifier_avatar}" />
