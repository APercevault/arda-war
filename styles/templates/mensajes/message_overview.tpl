...
<div id="titulo_alternativo_corto">
    <ul class="tabsbelow">
        <li>
              <span><font color = yellow size = 2><b>{$mg_overview}</b></font></span>
        </li>                                    
    </ul>
</div>
<div id="eins">
...
<td><b><a href="#" onclick="Message.getMessages({$MessID});return false;" style="color:{$MessInfo.color};"><font size = 2>{$MessInfo.lang}</a></b></td>
<td id="messcount" style="color:{$MessInfo.color};"><span id="unread_{$MessID}"><font size = 2>{$MessInfo.unread}</span>/<span id="total_{$MessID}">{$MessInfo.total}</span></td>
</tr>
{/foreach}
</table>
</div>	
