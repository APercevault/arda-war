...
function ReBuildView() {
        ...
        $.each(data.build, function(index, val) {
				...
				if (data.total == 1) {		
						...
						HTML    += '<br>'+val.name+'<br />'+data.nivel+' '+val.level+' '+(val.planet != '' ? ' @ '+val.planet : '')+'</b><div id="progressbar"></div>';
				if (data.total == 2 && index == 1) {
                        HTML    += '<a href="game.php?page=buildings&mode=research&amp;cmd=remove&amp;listid='+(index+1)+'"><div class="cancelar_c_small1">'+data.bd_cancel+'</div></a>';
									 }
