...
function refreshFormData() {
	...
	seconds -= minutes * 60;
$("#duration").text(hours + ("H : " + dezInt(minutes, 2) + "m : " + dezInt(seconds,2) + "s"));
	$("#distance").text(NumberGetHumanReadable(distance));
	$("#maxspeed").text(NumberGetHumanReadable(data.maxspeed));
	if (cargoSpace >= 0) {
		$("#consumption").html(NumberGetHumanReadable(consumption));
		$("#storage").html(NumberGetHumanReadable(cargoSpace));
	} else {
