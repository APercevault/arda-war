...
switch($action)
{
	...
	case 'fleet1':
		...
		$amiral = $db->countquery("SELECT `admiral` FROM ".USERS." WHERE `admiral` = '1';");
		if ($amiral == 0)
			{
				if ($ActualFleets >= floor(sqrt($_SESSION['USER'][$resource[124]])))
				exit($LNG['fl_expedition_fleets_limit']);
			}
		if ($amiral == 1)
			{
				if ($ActualFleets >= floor(sqrt($_SESSION['USER'][$resource[124]])) + 2)
				exit($LNG['fl_expedition_fleets_limit']);
			}
