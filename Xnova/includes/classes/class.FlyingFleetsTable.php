...
class FlyingFleetsTable
{
	 ...
	 private function CreateFleetPopupedMissionLink($FleetRow, $Texte, $FleetType)
   {
	...
	$Fleetsold = explode(" ", $FleetRow['fleet_array']);	
	if ($Fleetsold[0] < 230)	{	
	...
	if ($Fleetsold[0] > 220)	{	
		$FleetContent   = $this->CreateFleetPopupedFleetLink($FleetRow, (($MissionType == 1 || $MissionType == 2) && $FleetRow['fleet_owner'] != $_SESSION['id'] && $Status == 0 && $Owner == true) ? $LNG['cff_acs_fleet'] : $LNG['ov_fleet1'], $FleetPrefix.$FleetStyle[$MissionType]);}	
