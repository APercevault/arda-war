...
if($explo == 2)		{$Message 	= $LNG['sys_expe_found_dma_'.mt_rand(1, 3).'_'.mt_rand(1, 2).''];}
if($explo == 1)		{$Message 	= $LNG['sys_expe_found_dm_'.mt_rand(1, 3).'_'.mt_rand(1, 2).''];}	
		} else {
if($explo == 2)		{$Message 	= $LNG['sys_expe_nothinga_'.mt_rand(1, 9)];}	
if($explo == 1)		{$Message 	= $LNG['sys_expe_nothing_'.mt_rand(1, 9)];}
		}
	...
	function ReturnEvent()
	{
		global $LANG;
		$LNG			= $LANG->GetUserLang($this->_fleet['fleet_owner']);
if($explo == 2)		{if($this->_fleet['fleet_resource_darkmatter'] > 0) {
			SendSimpleMessage($this->_fleet['fleet_owner'], 0, $this->_fleet['fleet_end_time'], 15, $LNG['sys_mess_tower'], $LNG['sys_expe_report'], sprintf($LNG['sys_expe_back_home_with_dma'], $LNG['Darkmatter'], pretty_number($this->_fleet['fleet_resource_darkmatter']), $LNG['Darkmatter']));
			#$this->UpdateFleet('fleet_array', '220,0;');
		} else {
			SendSimpleMessage($this->_fleet['fleet_owner'], 0, $this->_fleet['fleet_end_time'], 15, $LNG['sys_mess_tower'], $LNG['sys_expe_report'], $LNG['sys_expe_back_home_without_dma']);
		}}
				
if($explo == 1)		{if($this->_fleet['fleet_resource_darkmatter'] > 0) {
