...
abstract class EtoileFunctions 
{
	...
	{
		global $resource;
		if ($CurrentUser['admiral'] >= 1) {
		return (1 + $CurrentUser[$resource[108]] + 2);
		} else {
	...
	public static function GotoEtoilePage()
	{	
		$temp = debug_backtrace();
		#redirectTo("game.php"."?page=etoile".($GLOBALS['CONF']['debug'] == 1 ? "&alert=".urlencode(str_replace($_SERVER["DOCUMENT_ROOT"],'.',$temp[0]['file'])." on ".$temp[0]['line']) : '')); Viejo
		if($GLOBALS['CONF']['debug'] == 1)
			FirePHP::getInstance(true)->log(str_replace($_SERVER["DOCUMENT_ROOT"],'.',$temp[0]['file'])." on ".$temp[0]['line']);
		redirectTo("game.php?page=etoile");	
	}
	...
	public static function GetAKSPage($CurrentUser, $CurrentPlanet, $fleetid)
	{
		...
		{
			$rand 			= mt_rand(100000,(int) 999999999);
			$aks_code_mr 	= "AG".$rand;
		...
		if(!empty($addname))
		{
			...
			foreach(explode(",", $aks['eingeladen']) as $a => $b)
			{
				if (!empty($b) && $added_user_id_mr == $b)
					redirectTo("game.php?page=etoile&action=getakspage&fleetid=".$daten['fleet_id']);
			}
	...
	public static function GetAvailableMissions($MissionInfo)
	{
		...
		if ($MissionInfo['planet'] == (MAX_PLANET_IN_SYSTEM + 1) && !CheckModule(55))
		...
		elseif ($MissionInfo['planettype'] == 2) {
			...
			if (!$UsedPlanet) {
				...
				if (!$YourPlanet && self::OnlyShipByID($MissionInfo['Ship'], 235) && !CheckModule(49))
					$missiontype[6] = $LNG['type_mission'][21];
				...
				elseif(!CheckModule(54)) {
					$missiontype[4] = $LNG['type_mission'][4];
				}
					
				if (!empty($MissionInfo['IsAKS']) && !$YourPlanet && !CheckModule(50))
