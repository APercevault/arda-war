...
class StringParser {
	...
	function parse ($text) {
		...
		$res = StringParser_Node1::destroyNode ($this->_root);
		if (!$res) {
			$this->_parsing = false;
			return false;
		}
...
class StringParser_Node1 {
	...
	function appendToLastTextChild ($text) {
		...
		if ($ccount == 0 || $this->_children[$ccount-1]->_type != STRINGPARSER_NODE_TEXT) {
			$ntextnode = new StringParser_Node_Text1 ($text);
	...
	function removeChild (&$child, $destroy = false) {
		...
		if ($destroy) {
			return StringParser_Node1::destroyNode ($object);
			unset ($object);
		}
		return true;
	}
 ...
 * @package stringparser
 */
class StringParser_Node_Root extends StringParser_Node1 {
	/**
	 * The type of this node.
	 * 
 ...
 * @package stringparser
 */
class StringParser_Node_Text1 extends StringParser_Node1 {
	/**
	 * The type of this node.
	 * 
