...
if (!defined('CLI') && !defined('LOGIN') && !defined('IN_CRON') && !defined('AJAX') && !defined('ROOT'))
{
	...
	if($CONF['game_disable'] == 0 && $_SESSION['authlevel'] == AUTH_USR) {
		message("<font size=\"6px\">".$CONF['close_reason']);
	}
	...
	if($USER['bana'] == 1)
	{
		message("<font size=\"6px\">".$LNG['css_account_banned_message']."</font><br><br><font size=\"6px\">".sprintf($LNG['css_account_banned_expire'],date("d/m/Y --- H:i ", $USER['banaday']))."<br>".$LNG['css_goto_homeside']);
		exit;
	}
