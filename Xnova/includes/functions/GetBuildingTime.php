...
if(!defined('INSIDE')) die('Hacking attempt!');
	...
	function GetBuildingTime ($USER, $PLANET, $Element, $Destroy = false)
	{
		...
		if($USER['commander'] >= 1 xor $USER['raza'] == 0) {		
			$tiempo	= $Cost / ($CONF['game_speed'] * (2 + $PLANET[$resource[14]])) * pow(0.5, $PLANET[$resource[15]]);	
		...
		} } elseif (in_array($Element, $reslist['tech']))
		{
			...
			if($USER['commander'] >= 1 and $USER['technocratic'] >= 1) {
				...
				@$tiempo			= $Cost / (1000 * ($Level * 2)) / ($CONF['game_speed'] / 2500) * pow(1 - UNIVERISTY_RESEARCH_REDUCTION, $PLANET[$resource[6]]);
				...
				@$time			= $Cost / (1000 * ($Level * 2)) / ($CONF['game_speed'] / 2500) * pow(1 - UNIVERISTY_RESEARCH_REDUCTION, $PLANET[$resource[6]]);
			}			
        }
