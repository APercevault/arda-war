...
class ShowOfficierPage
{
 ...
 if ( $Officier == 604 && $USER['technocratic'] == 0 ) { $db->query("UPDATE ".USERS." SET `spy_tech`=spy_tech+2 WHERE id=".$USER['id']."");}
	...
	$template->message("<font color=lime size=4>".$LNG['oficial_contratado'],"?page=oficiales", 2);
   ...
   {      
	...
	if ($USER['commander'] <= 0 and $USER['darkmatter'] >= $pricelist[600]['week']['darkmatter']) {
	...
	if ($USER['commander'] <= 0 and $USER['darkmatter'] >= $pricelist[600]['months']['darkmatter']) {
	...
	if ($USER['engineer'] <= 0 and $USER['darkmatter'] >= $pricelist[603]['week']['darkmatter']) {
	...
	if ($USER['engineer'] <= 0 and $USER['darkmatter'] >= $pricelist[603]['months']['darkmatter']) {
	...
	if ($USER['geologe'] <= 0 and $USER['darkmatter'] >= $pricelist[601]['week']['darkmatter']) {
	...
	if ($USER['geologe'] <= 0 and $USER['darkmatter'] >= $pricelist[601]['months']['darkmatter']) {
	...
	if ($USER['technocratic'] <= 0 and $USER['darkmatter'] >= $pricelist[604]['week']['darkmatter']) {
	...
	if ($USER['technocratic'] <= 0 and $USER['darkmatter'] >= $pricelist[604]['months']['darkmatter']) {
	...
	if ($USER['admiral'] <= 0 and $USER['darkmatter'] >= $pricelist[602]['week']['darkmatter']) {
	...
	if ($USER['admiral'] <= 0 and $USER['darkmatter'] >= $pricelist[602]['months']['darkmatter']) {
