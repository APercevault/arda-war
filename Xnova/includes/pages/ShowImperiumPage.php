...
function ShowImperiumPage()
{
	...
	while ($Planet = $db->fetch_array($PlanetsRAW))
	{
		$InfoList	= array(
			...
			'metal'			=> shortly_number($Planet['metal']),
			'crystal'		=> shortly_number($Planet['crystal']),
			'deuterium'		=> shortly_number($Planet['deuterium']),
			'norio'		    => shortly_number($Planet['norio']),
			'energy_used'	=> shortly_number($Planet['energy_max'] + $Planet['energy_used']),
			'energy_max'	=> shortly_number($Planet['energy_max']),
