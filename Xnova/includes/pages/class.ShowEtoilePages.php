...
require_once(ROOT_PATH . 'includes/classes/class.EtoileFunctions.php');

class ShowEtoilePages extends EtoileFunctions
{

	public static function ShowEtoilePage()
	{
		...
		if ($MaxExpedition >= 1)
		{
			...
			if ($USER['admiral'] >= 1) {$EnvoiMaxExpedition = floor(sqrt($MaxExpedition)) + 2;}
		}
		else
		{
			$ExpeditionEnCours 	= 0;
...
$explo = 2 ;			
		...
		foreach($reslist['soldatz'] as $FleetID)
		{
...
$PE_autreplapla  	= $db->uniquequery("SELECT `p_etoile` FROM `".PLANETS."` WHERE `galaxy` = '".$TargetGalaxy."' AND `system` = '".$TargetSystem."' AND `planet` = '".$TargetPlanet."';");

if ($TargetPlanet == 16) {$PE_autreplapla['p_etoile'] = 16 ;}
$PE_maplapla					= $PLANET['p_etoile'];


		$template->assign_vars(array(
		
			'fa_act_porte'			=> $LNG['fa_act_porte'],		
			'fa_votre_no_porte'		=> $LNG['fa_votre_no_porte'],		
			'fa_no_porte'			=> $LNG['fa_no_porte'],
			'PE_autreplapla'		=> $PE_autreplapla['p_etoile'],
			'PE_maplapla'			=> $PE_maplapla,
			...
			'pe_new_mission_title'	=> $LNG['pe_new_mission_title'],
			...
			'fl_selection'			=> $LNG['fl_selection'],
			'pe_no_ships'			=> $LNG['pe_no_ships'],
			'pe_select_all_Soldats'	=> $LNG['pe_select_all_Soldats'],
		...
		$template->show('porte/Etoile_table.tpl');
	}

	public static function ShowEtoile1Page()
	{
		...
		foreach ($reslist['soldatz'] as $id => $ShipID)
		{
		...
		$template->assign_vars(array(
			...
			'fl_send_equipe'		=> $LNG['fl_send_equipe'],
			'fl_destiny'			=> $LNG['fl_destiny'],
		...
		$template->show('porte/Etoile1_table.tpl');
	}
	
	public static function ShowEtoile2Page()
	{
		...
		if($TargetPlanettype == 2)
		{
			...
			{
				$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_no_empty_derbis']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}	
		}
		...
		{
			$template->message("<font color=#FF0000 size=4><b>". $LNG['fl_empty_target']."</b></font>", "game.php?page=etoile", 2);
			exit;
		}
		...
		{
			$template->message("<font color=#FF0000 size=4><b>". sprintf($LNG['fl_no_enought_deuterium'], $LNG['Deuterium'], pretty_number($PLANET['deuterium'] - $consumption), $LNG['Deuterium'])."</b></font>", "game.php?page=etoile", 2);
			exit;
		}
...
if ($TargetPlanet == 16) {$PE_autreplapla['p_etoile'] = 16 ;}
		...
		$template->assign_vars(array(
			...
			'fl_expedition_alert_message1'	=> $LNG['fl_expedition_alert_message1'],
		...
		$template->show('porte/Etoile2_table.tpl');
	}

	public static function ShowEtoile3Page()
	{
		...
		if (IsVacationMode($USER))
			exit($template->message("<font color=#FF0000 size=4>".$LNG['fl_vacation_mode_active'], 'game.php?page=overview', 2));

		if ($_SESSION['last']['user_side'] != 'game.php?page=etoile2')
			parent::GotoEtoilePage();
		...
		{
			$template->message("<font color=lime size=4><b>".$LNG['fl_empty_transport']."</b></font>", "game.php?page=etoile", 10);
			exit;
		}
		...
		{
			$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_no_slots'], "game.php?page=etoile", 1);
			exit;
		}
		...
		if ($mission == 11)
		{
			...
			{
				$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_expedition_fleets_limit']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}
		}
		elseif ($mission == 15)
		{
			...
			if ($MaxExpedition == 0)
			{
				$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_expedition_tech_required']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}			
		...
		{
			$template->message("<font color=lime size=4><b>".$LNG['fl_in_vacation_player']."</b></font>", "game.php?page=etoile", 2);
			exit;
		}
		...
		if(!$YourPlanet && ($mission == 1 || $mission == 2 || $mission == 5 || $mission == 6 || $mission == 9))
		{
			...
			{
				$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_admins_cannot_be_attacked']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}
			...
			{
				$template->message("<font color=lime size=4><b>".$LNG['fl_week_player']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}
			elseif ($IsNoobProtec['StrongPlayer'])
			{
				$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_strong_player']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}
		}
		...
		if ($mission == 5)
		{
			...
			{
				$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_no_same_alliance']."</b></font>", "game.php?page=etoile", 2);
				exit;
			}		
		}
		...
		{
			$template->message("<font color=#FF0000 size=4><b>".sprintf($LNG['fl_no_enought_deuterium'], $LNG['Deuterium'], pretty_number($consumption - $PLANET['deuterium']), $LNG['Deuterium'])."</b></font>", "game.php?page=etoile", 2);
			exit;
		}
		
		if ($StorageNeeded > $FleetStorage)
		{
			$template->message("<font color=#FF0000 size=4><b>".$LNG['fl_no_enought_cargo_capacity'] . pretty_number($StorageNeeded - $FleetStorage)."</b></font>", "game.php?page=etoile", 2);
			exit;
		}
...
if ($TargetPlanet == 16) {$PE_autreplapla['p_etoile'] = 16 ;}
		...
		$template->show('porte/Etoile3_table.tpl');
	...
	public static function FleetAjax()
	{
		...
		$UserSpyProbes  = $PLANET[$resource[235]];
		...
		$CurrentFlyingFleets = parent::GetCurrentFleets($USER['id']);	
			...
			$TargetPlanet					= request_var('planet', $PLANET['planet']);		
			$PE_autreplapla  	= $db->uniquequery("SELECT `p_etoile` FROM `".PLANETS."` WHERE `galaxy` = '".$TargetGalaxy."' AND `system` = '".$TargetSystem."' AND `planet` = '".$TargetPlanet."';");
		if ($PE_autreplapla['p_etoile'] < 1)
		{
			$ResultMessage = "602; ".$LNG['fa_no_porte']." |".$CurrentFlyingFleets." ".$UserSpyProbes." ".$UserRecycles." ".$UserGRecycles." ".$UserMissiles;
			die($ResultMessage);
		}			
		if ($PE_autreplapla['p_etoile'] > 0)					
		
		switch($mission)
		{
			case 6:
				$SpyProbes	= request_var('ships', 0);
				$SpyProbes	= min($SpyProbes, $PLANET[$resource[235]]);
				if(empty($SpyProbes))
					exit($ResultMessage = "611; ".$LNG['fa_no_drone']." |".$CurrentFlyingFleets." ".$UserSpyProbes." ".$UserRecycles." ".$UserGRecycles." ".$UserMissiles);
					
				$FleetArray = array(235 => $SpyProbes);
			break;
			case 8:
	...
	{	
		...
		elseif (!in_array($pziel, $reslist['defense1']) && $pziel != 0)
		...
		if ($error != "")
		{
			$template->message("<font color=#FF0000 size=4><b>".$error);
			exit;
		}
		$SpeedFactor    	 = parent::GetGameSpeedFactor();
