...
class ShowChatPage
{
	...
	private function DelMeassageFromChat($MessageID) {
		...
		$db->query("DELETE FROM ".CHAT." WHERE `timestamp` = '".$MessageID."';");
...
$msg = str_replace("'","\'",$msg); 			
	...
	private function GetMessages($chat_type) {
		...
		$Chat 	= $db->query("SELECT * FROM ".CHAT." WHERE `universe` = '".$UNI."' AND `timestamp` > '".$TimeStamp."' AND ally_id = '".(($chat_type == "ally") ? $USER['ally_id'] : 0)."' ORDER BY timestamp DESC LIMIT ".$this->page_limit.";");
		$msg 	= array();
		
		while($Message = $db->fetch_array($Chat)){
			$msg[$Message['timestamp']] = array(
				'date'	=> date("m/d ".TIMEFORMAT, $Message["timestamp"]),
