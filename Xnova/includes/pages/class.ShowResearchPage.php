...
class ShowResearchPage
{
	...
	{
		global $PLANET, $USER, $db, $resource, $removea, $rese;
...
$removea = 0 ;		
 ...
 header( "refresh:0" );		
	...
	{
		global $USER, $PLANET, $db, $PLANETA, $removeb, $rese;
...
$removeb = 0 ;
     if(($PLANET['id'] == $USER['b_tech_planet']) && ($USER['b_tech_planet'] == $PLANETA))
      {
         ...
         $db->query("UPDATE ".PLANETS." SET `metal` = `metal` + '".$Resses['metal']."', `crystal` = `crystal` + '".$Resses['crystal']."', `deuterium` = `deuterium` + '".$Resses['deuterium']."',`norio` = `norio` + '".$Resses['norio']."' WHERE `id` = '".$PLANETA."';");
     }
	...
	{
		global $PLANET, $USER, $resource, $db, $rese;
$rese = 1;			
		$CurrentQueue  		= unserialize($USER['b_tech_queue']);
 ...
 /*        }
            ...
            $db->query("UPDATE ".PLANETS." SET `metal` = `metal` - '".$Resses['metal']."', `crystal` = `crystal` - '".$Resses['crystal']."', `deuterium` = `deuterium` - '".$Resses['deuterium']."',`norio` = `norio` - '".$Resses['norio']."' WHERE `id` = '".$PLANET['id']."';");
        }
*/		
         $USER['darkmatter']         -= $Resses['darkmatter'];
	...
	{
		global $PLANET, $USER, $LNG, $resource, $reslist, $CONF, $db, $pricelist, $rese;
		...
		if ($PLANET[$resource[31]] == 0)
		{
			...
			$template->message("<font color=#FF0000 size=4>".$LNG['bd_lab_required']);
			exit;
		}
...
$total = 	count($TechQueue);	
$allow = $PLANET['labo'];				
if($_GET['mode1'] == 1){$allow ="tech" ;	$db->query("UPDATE ".PLANETS." SET labo = 'tech' ;");}
if($_GET['mode1'] == 2){$allow ="tech1" ;	$db->query("UPDATE ".PLANETS." SET labo = 'tech1' ;");}
if($_GET['mode1'] == 3){$allow ="propulsion" ;	$db->query("UPDATE ".PLANETS." SET labo = 'propulsion' ;");}
if($_GET['mode1'] == 4){$allow ="arme" ; $db->query("UPDATE ".PLANETS." SET labo = 'arme' ;");}
if($_GET['mode1'] == 5){$allow ="extration" ;	$db->query("UPDATE ".PLANETS." SET labo = 'extration' ;");}	
		foreach($reslist[$allow] as $ID => $Element)
		{
...
$rese = 1;					
		...
		$skin_raza = "voltra";
			...
			if(isset($pricelist[$Element]['max']) && $USER[$resource[$Element]] >= $pricelist[$Element]['max'] && IsTechnologieAccessible($USER, $PLANET, $Element)) {
...
$rese = 1;			
		...
		$template->assign_vars(array(
				        ...
				        'bv_gene'                 => $LNG['bv_gene'],
						'bv_tech'                 => $LNG['bv_tech'],
						'bv_propul'                 => $LNG['bv_propul'],
						'bv_arme'                => $LNG['bv_arme'],
						'bv_extra'                 => $LNG['bv_extra'],			
