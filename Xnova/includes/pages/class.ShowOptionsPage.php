...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		case 'avatar':
								$template->assign_vars(array(
									...
									'hauteur'  			 => HEIGHT_MAX1,								
									'largeur'  			 => WIDTH_MAX1,	
									'ally_image'				=> $USER['avatar'],
									'al_manage_image'  			 => $LNG['op_avatar'],
									'al_change_logo'  			 => $LNG['op_change_avatar'],
									'al_back'				=> $LNG['al_back'],
									'fl_continue'			=> $LNG['fl_continue'],									
								));	
								$template->show("avatar/avatar_joueur.tpl");		  
...
$longueur = strlen($newpass1);		
            
            if ($longueur > 3 && md5($db_password) == $USER["password"] && $newpass1 == $newpass2)
            {
...
$rep1  = $db->uniquequery("SELECT id FROM ".USERS." WHERE `id`= '".$USER['id']."';");
$rep = $rep1[id] . "/";
@mkdir(TURGET);
if( !is_dir(TURGET.$rep) ) {
  if( !mkdir(TURGET.$rep, 0755) ) {
	...
	$template->message("<font color=\"lime\" size=\"4\"><b>".$LNG['al_logo_introuva'],"?page=options&mode=avatar",4);
                exit; 		
    }
      // On recupere les dimensions du fichier
      $infosImg = getimagesize($_FILES['fichier']['tmp_name']);
        // On verifie les dimensions et taille de l'image
        if(($infosImg[0] > WIDTH_MAX1) or ($infosImg[1] > HEIGHT_MAX1) or (filesize($_FILES['fichier']['tmp_name']) > MAX_SIZE1))	  
			
			{  
	$template->message("<font color=\"lime\" size=\"4\"><b>".$LNG['al_logo_h_c'],"?page=options&mode=avatar",7);
                exit; 
			;}
    ...
    if( !strstr($type_file, 'jpg') && !strstr($type_file, 'jpeg')  && !strstr($type_file, 'gif')&& !strstr($type_file, 'png') )
    {
		...
		$template->message("<font color=\"lime\" size=\"4\"><b>".$LNG['al_logo_no_image'],"?page=options&mode=avatar",7);
                exit;
    }
...
$file1 = pathinfo($_FILES['fichier']['name'], PATHINFO_FILENAME);
    if( !ctype_alnum($file1) )
    {
		...
		$template->message("<font color=\"lime\" size=\"4\"><b>".$LNG['al_name_image'],"?page=options&mode=avatar",7);
                exit;
    }
	...
	array_map('unlink', glob(TURGET.$rep ."*.jpg"));
	array_map('unlink', glob(TURGET.$rep ."*.jpeg"));
	array_map('unlink', glob(TURGET.$rep ."*.gif"));
	array_map('unlink', glob(TURGET.$rep ."*.png"));	

    if( !move_uploaded_file($tmp_file, TURGET.$rep . $name_file) )
    {
		$template->message("<font color=\"lime\" size=\"4\"><b>".$LNG['al_logo_imposs'],"?page=options&mode=avatar",7);
                exit; 	
	...
	$name_file1 = TURGET1.$rep . $name_file ;

$db->query("UPDATE ".USERS." SET avatar ='".$name_file1."' WHERE `id`= '".$USER['id']."';"); 	
	$template->message("<font color=\"lime\" size=\"4\"><b>".$LNG['al_logo_created'],"?page=options&mode=avatar",7);
                exit; 
}		 
            ...
            else
            {
               ...
               $template->assign_vars(array(   
				  ...
				  'op_modifier_avatar'               => $LNG['op_modifier_avatar'],			   
                  'user_avatar'               => $USER['avatar'],			   
				  'op_avatar'               => $LNG['op_avatar'],
