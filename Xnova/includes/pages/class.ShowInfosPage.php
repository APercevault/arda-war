...
class ShowInfosPage
{
	private function GetNextJumpWaitTime($CurMoon, $ReturnString = false)
	{
		...
		$RestWait   = 0;
		$RestString = "";		
		$NextJumpTime1  = JUMPGATE_WAIT_TIME-(JUMPGATE_REDUCTION_PER_LEVEL*($CurMoon['sprungtor']-1));
		if ($NextJumpTime1 < JUMPGATE_MAX_REDUCTION){$NextJumpTime1 = JUMPGATE_MAX_REDUCTION;}
		$NextJumpTime   = $LastJumpTime + $NextJumpTime1;
	...
	private function DoFleetJump()
	{
		...
		return json_encode(array('message' =>  $LNG['in_jump_gate_done'].' '. $this->GetNextJumpWaitTime($PLANET, true), 'error' => false));
	}
