...
function ShowSearchPage()
{
	...
	switch($type) {
		case 'playername':
$longueur = strlen($searchtext);			
if($longueur > 3) {			
			$search = $db->query("SELECT 
              a.id, a.username, a.ally_id, a.galaxy, a.system, a.planet, a.avatar, b.name, c.total_rank, d.ally_name
              FROM ".USERS." as a                                                                   INNER JOIN ".PLANETS." as b ON b.id = a.id_planet 
              LEFT JOIN ".STATPOINTS." as c ON c.id_owner = a.id AND c.stat_type = 1
              LEFT JOIN ".ALLIANCE." as d ON d.id = a.ally_id
              WHERE a.`universe` = '".$UNI."' AND a.username LIKE '%".$db->sql_escape($searchtext, true)."%' LIMIT 1000;");
...
if($longueur > 3) {		
			 $search = $db->query("SELECT 
                                                                        ...
                                                                        WHERE a.`universe` = '".$UNI."' AND a.name LIKE '%".$db->sql_escape($searchtext, true)."%' LIMIT 1000;");
			while($s = $db->fetch_array($search))
			{
				$SearchResult[]	= array(
					'planetname'	=> $s['name'],
		...
		break;
		case "allytag":
	  $search = $db->query("SELECT a.id, a.ally_name, a.ally_image, a.ally_tag, a.ally_members, c.total_points FROM ".ALLIANCE." as a LEFT JOIN ".STATPOINTS." as c ON c.stat_type = 2 AND c.id_owner = a.id WHERE a.`ally_universe` = '".$UNI."' AND a.ally_tag LIKE '%".$db->sql_escape($searchtext, true)."%' LIMIT 1000;");
			while($s = $db->fetch_array($search))
			{
				$SearchResult[]	= array(
	...
	$template->assign_vars(array(
	  ...
	  $search = $db->query("SELECT a.id, a.ally_name, a.ally_image, a.ally_tag, a.ally_members, c.total_points FROM ".ALLIANCE." as a LEFT JOIN ".STATPOINTS." as c ON c.stat_type = 2 AND c.id_owner = a.id WHERE a.`ally_universe` = '".$UNI."' AND a.ally_name LIKE '%".$db->sql_escape($searchtext, true)."%' LIMIT 1000;");
			while($s = $db->fetch_array($search))
			{
				$SearchResult[]	= array(
					'allypoints'	=> pretty_number($s['total_points']),
