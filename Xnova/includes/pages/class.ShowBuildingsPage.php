...
{    
      public function GetRestPrice($Element)
        ...
        {
                global $PLANET, $USER, $buld,$removea ;
	$buld = 1;			
...
$removea = 0 ;					
 ...
 header( "refresh:0" );		
        ...
        {
                global $USER, $PLANET, $db, $removeb, $buld;
$buld = 1; 				
...
$removeb = 0 ;
        ...
        {
                global $PLANET, $USER, $resource, $buld;
	$buld = 1;                        
                $CurrentQueue           = unserialize($PLANET['b_building_id']);
        ...
        {
                global $LNG, $CONF, $PLANET, $USER, $buld;
	$buld = 1;                
        ...
        {
                global $ProdGrid, $LNG, $resource, $reslist, $CONF, $db, $PLANET, $USER, $pricelist, $buld, $bulda, $buldo	;
	$buld = 1;
	$bulda = 3;
        ...
        $ListID         = request_var('listid',0);
               ...
               if(!empty($Element) && $USER['urlaubs_modus'] == 0 && ($PLANET[$resource[$Element]] < $pricelist[$Element]['max'] && IsTechnologieAccessible($USER, $PLANET, $Element) && in_array($Element, $reslist['allow'][$PLANET['planet_type']])) || $TheCommand == "cancel" || $TheCommand == "remove")
                {
...
$allow = $PLANET['buil'];				
if($_GET['mode'] == 1){$allow ="allow" ;	$db->query("UPDATE ".PLANETS." SET buil = 'allow' ;");}
if($_GET['mode'] == 2){$allow ="ressource" ;	$db->query("UPDATE ".PLANETS." SET buil = 'ressource' ;");}
if($_GET['mode'] == 3){$allow ="stockage" ;	$db->query("UPDATE ".PLANETS." SET buil = 'stockage' ;");}
if($_GET['mode'] == 4){$allow ="infrastruture" ; $db->query("UPDATE ".PLANETS." SET buil = 'infrastruture' ;");}
if($_GET['mode'] == 5){$allow ="lune" ;	$db->query("UPDATE ".PLANETS." SET buil = 'lune' ;");}
		...
		$skin_raza = "voltra";
  			...
  			if(isset($pricelist[$Element]['max']) && $PLANET[$resource[$Element]] >= $pricelist[$Element]['max']) {
				$BulidLink  =	'<img class="tooltip" name="<table><td>'.$LNG['bd_maxlevel'].'</td></table>" src="styles/theme/' .$skin_raza .'/imagenes/navegacion/construir_red.gif" />';  }								
                        $BuildInfoList[]        = array(
                                'id'                    => $Element,
                ...
                if ($PLANET['b_building'] != 0)
                {
                        ...
                        $template->assign_vars(array(
                                ...
                                'data'                          => json_encode(array('total' => $total,'bd_cancel' => $LNG['bd_cancel'], 'bd_continue' => $LNG['bd_continue'], 'bd_finished' => $LNG['bd_finished'], 'build' => $Queue, 'nivel' => $LNG['bd_lvl'], 'raza_skin' => $skin_raza  )),
                        ));
                }
