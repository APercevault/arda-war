	...
	case 'lostpassword': 
		if ($mode == "send") {
			...
			if (empty($ExistMail['username'])) {
				$template->message("<font color=#FF0000 size=4>".$LNG['mail_not_exist'], "index.php?page=lostpassword&lang=".$LANG->getUser(), 3, true);
			} else {
				...
				if(true === true)
				{
					...
					$template->message("<font color=lime size=4>".$LNG['mail_sended'], "./?lang=".$LANG->getUser(), 5, false);
				} else {
					$template->message("<font color=#FF0000 size=4>".$LNG['mail_sended_fail'], "./?lang=".$LANG->getUser(), 5, true);
				}
		...
		switch ($mode) {
			case 'send' :
				...
				if (strlen($UserPass) > 15)
					$errors .= $LNG['password1_lenght_error'];					
...
$longueur = strlen($UserPlanet);				
				if ($longueur <4 or $longueur >15)
					$errors .= (UTF8_SUPPORT) ? $LNG['planet_field_no_space'] : $LNG['planet_field_no_alphanumeric'];			
								
				if (!empty($errors)) {
					$template->message("<font color=#FF0000 size=4>".$errors, '?page=reg&lang='.$LANG->getUser(), 3, true);
					exit;
				}
				...
				if($CONF['user_valid'] == 0 || $CONF['mail_active'] == 0) {
					...
					$template->message("<font color=lime size=4>".$LNG['reg_completed'], '?lang='.$UserLang, 10, true);
				}								
			break;
			case 'valid' :		
				...
				while ($PlanetID === false) {
							 $Planet = mt_rand(1,(int) MAX_PLANET_IN_SYSTEM);				  //15
							 $LastSettedGalaxyPos = mt_rand(1,(int) MAX_GALAXY_IN_WORLD);   //9

if($LastSettedGalaxyPos >1)	{$LastSettedSystemPos = mt_rand(1,  (int) MAX_SYSTEM_IN_GALAXY);} // 499

else					    {$LastSettedSystemPos = mt_rand(2,(int) MAX_SYSTEM_IN_GALAXY);} // 499									
